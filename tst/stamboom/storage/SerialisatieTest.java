...
public class SerialisatieTest extends TestCase {
    ...
    public void testOpslag() {
        ...
        assertEquals("nummering van personen onjuist", adm.aantalGeregistreerdePersonen() +1, jan.getNr()); //Question
        Gezin rosaEnJan = adm2.addOngehuwdGezin(rosa, jan);
        assertEquals("nummering van gezinnen onjuist", adm.aantalGeregistreerdeGezinnen() +1, rosaEnJan.getNr()); //Question

        assertEquals("aantal personen in observable personen onjuist", adm.aantalGeregistreerdePersonen() +1,
                adm2.getPersonen().size());
        
        assertTrue(testOpslag.delete());  //Question
    }
}
